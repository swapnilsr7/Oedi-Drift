...
Runs on a schedule via GitHub Actions. For each source in data/sources.json:
  ...
  4. Purges items matching a source's exclude_keywords, checking BOTH the
     title and the article URL path (a horoscope item's title is often
     just the zodiac sign — "Aries", "Virgo" — the word "horoscope" only
     appears in the URL, e.g. /horoscope/daily/aries).
  5. Sends new items to Claude for summary, category (pinned per-source or
     AI-decided), and style tags. Uses a cheaper model by default to keep
     ongoing API cost low. If an image URL is blocked, retries text-only.
  6. Repairs previously broken items (Uncategorized + no summary) a batch
     per run, so temporary API failures heal automatically.
  7. Writes data/index.json and data/last_run.json with full run stats.

Per-source options in sources.json:
  "category": "Jewellery"            -> pin every item's category
  "exclude_keywords": ["horoscope"]  -> purge matching items (title OR URL)
"""
...
# Cheaper model for routine tagging/summarizing. Cuts ongoing API cost
# substantially versus a Sonnet-class model, at some loss of nuance in
# summaries and tag specificity — a reasonable trade for a personal archive.
...
# Always-purged noise regardless of per-source config. Zodiac signs alone
# (without the word "horoscope") are the classic miss: a card titled
# "Aries" or "Virgo" is a horoscope item even though it doesn't say so.
GLOBAL_EXCLUDE_TITLE_WORDS = [
    "horoscope", "tarot", "zodiac", "astrology",
    "aries", "taurus", "gemini", "cancer", "leo", "virgo",
    "libra", "scorpio", "sagittarius", "capricorn", "aquarius", "pisces",
]
GLOBAL_EXCLUDE_URL_WORDS = ["horoscope", "tarot", "zodiac", "astrology"]
...
def is_excluded(title, link, source_keywords):
    """
    True if this item should be purged/skipped. Checks the item's title
    AND its URL path against both the source-specific keyword list and
    the always-on global noise list (horoscopes, zodiac signs, etc).

    Zodiac sign names are matched as WHOLE WORDS ONLY (word boundaries),
    since "leo", "cancer", "gemini" etc are common words/names elsewhere
    (a person named Leo, a cancer-care hospital design feature, Google's
    Gemini) and a plain substring match would wrongly purge those.
    Explicit astrology terms (horoscope, tarot, zodiac, astrology) are
    still matched as substrings, since false positives there are rare.
    """
    title_low = (title or "").lower()
    try:
        path_low = urlparse(link or "").path.lower()
    except Exception:
        path_low = (link or "").lower()

    source_words = set(k.lower() for k in source_keywords)
    explicit_title_words = source_words | {"horoscope", "tarot", "zodiac", "astrology"}
    explicit_url_words = source_words | {"horoscope", "tarot", "zodiac", "astrology"}

    if any(w in title_low for w in explicit_title_words):
        return True
    if any(w in path_low for w in explicit_url_words):
        return True
    if any(re.search(rf"\b{re.escape(sign)}\b", title_low) for sign in _ZODIAC_SIGNS):
        return True
    if any(re.search(rf"\b{re.escape(sign)}\b", path_low) for sign in _ZODIAC_SIGNS):
        return True
    return False
...
def main():
    ...
    source_keywords = {s["name"]: s.get("exclude_keywords", []) for s in sources}
    index = load_json(INDEX_PATH, [])

    # Purge items matching exclude rules retroactively (title OR URL path),
    # including the always-on horoscope/zodiac list, so a fix here also
    # cleans out everything already sitting in the archive.
    before_ex = len(index)
    index = [
        i for i in index
        if not is_excluded(i.get("title", ""), i.get("link", ""), source_keywords.get(i.get("source", ""), []))
    ]
    removed_excluded = before_ex - len(index)
    ...
    for src in sources:
        ...
        keywords = src.get("exclude_keywords", [])
        ...
        for entry in entries:
            ...
            if nl in known_links or tk in known_titles:
                continue
            if is_excluded(entry["title"], entry["link"], keywords):
                continue
