...
new items, repaired items, purged duplicates/shopping-links/excluded-items,
broken sources, and AI-analysis failures (e.g. exhausted API credit) with
...
def main():
    ...
    removed_excl = report.get("removed_excluded", 0)
    ...
    if removed_excl:
        lines.append(f"**Excluded/horoscope items purged from the archive:** {removed_excl}")
    lines.append("")
    ...
    if (report["new_items"] == 0 and not broken and not analysis_errors
            and not repaired and not removed_dupes and not removed_shop and not removed_excl):
